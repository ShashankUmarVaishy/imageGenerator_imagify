...
export const generateImage = async (c) => {
  try {
    ...
    if (!user || !prompt) {
      return c.json({ success: false, message: "user or prompt not found" });
    }
    ...
    return c.json({
      success: false,
      message: `${error.message} — this is the error from catch block of image controller`,
    });
  }
};
